...
def get_x(obj, mode='train'):
    ...
    folder_path = os.path.join(DATASET_PATH, f'{obj}/{mode}')
    
    # Überprüfen, ob der Ordner existiert
    if not os.path.exists(folder_path):
        raise FileNotFoundError(f"Der Ordner '{folder_path}' wurde nicht gefunden. Überprüfen Sie den Pfad.")
    else:
        print(f"Ordner gefunden: {folder_path}")
    ...
    if images.size == 0:
        raise ValueError("No images were loaded. Check the dataset path or file structure.")
